...
{
  public  class NotaSalidaRepositoy : Singleton<NotaSalidaRepositoy>, INotaSalidaRepositoy<NotaSalida, NotaSalidaDetalle, int>
    {
        #region Attributos
                ...
                using (var transaction = conexion.BeginTransaction(IsolationLevel.Serializable))
                {
                    try
                    {
                        using (var comando = _database.GetStoredProcCommand(string.Format("{0}{1}", ConectionStringRepository.EsquemaName, "SGE_NOTA_SALIDA_INSERT")))
                        {
                            ...
                            _database.AddInParameter(comando, "@nsalc_fecha_nota_salida", DbType.String, DateTime.ParseExact(entity.nsalc_fecha_nota_salida_,"dd/mm/yyyy",CultureInfo.InvariantCulture).ToString("yyyy/mm/dd"));
                        ...
                        foreach (var itemdetalle in entity.listaDetalleNS)
                        {
                            ...
                            {
                                _database.AddInParameter(comandoKardex, "@kardc_fecha_movimiento", DbType.String, DateTime.ParseExact(entity.nsalc_fecha_nota_salida_, "dd/mm/yyyy", CultureInfo.InvariantCulture).ToString("yyyy/mm/dd"));
